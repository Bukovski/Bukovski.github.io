...
$(function() {
    ...
    $('#my-menu').mmenu({
        ...
        navbar: {
            title: '<img src="img/logoColor.svg" alt="Салон красоты Смитлер">'
        },
        offCanvas: {
            position: "right"
        }
    ...
    //Карусель услуг с ценниками начало
    function carouselService() { //получает размер блока в каруселе и подгоняет под эти размеры картинки
    ...
    $('.carousel-services').on('initialized.owl.carousel', function () { //проверяем загрузилась ли курусель
        setTimeout(function () {
            carouselService(); //если карусель загружена генерируем размер избражений для нее
        }, 100);
    });
    //карусель
    $('.carousel-services').owlCarousel({
        loop: true, //бесконечно пролистывать
        nav: true,
        dots: false, //отключаем встроенную навигацию карусели (кружки)
        smartSpeed: 700,
        navText: ['<i class="fa fa-angle-double-left"></i>', '<i class="fa fa-angle-double-right"></i>'],
        resposiveClass: true, //включить адаптивность
        responsive: {
            0: {
                items: 1
            },
            ...
            1100: { //размер экрана больше 1100
                items: 3 //показать 3 слайда
            }
        }
    });
    
    $('.carousel-services-composition .h3').each(function () { //засовываем второе слово в спан (карусель услуг)
        var ths = $(this);
        ths.html(ths.html().replace(/(\S+)\s*$/, '<span>$1</span>'));
    });
    
    $('.carousel-services-content').equalHeights(); //автоматически подгоняет высоту блока под все остальные
    //Карусель услуг с ценниками конец
    
    $('section .h2').each(function () { //Выделяет только первое слово в строке оборачивая в SPAN
        var ths = $(this);
        ths.html(ths.html().replace(/^(\S+)/, '<span>$1</span>'));
    });
    
    $('select').selectize(); //поиск в выпадающей форме
    //E-mail Ajax Send
    $("form.callback").submit(function() { //Change
        ...
        $.ajax({
            ...
            $(th).find('.success').addClass('active').css('display', 'flex').hide().fadeIn(); //после отправки формы, форма пропадет, а на месте появится надпись
            setTimeout(function() {
                $(th).find('.success').removeClass('active').fadeOut(); //после отправки крываем надпись об отправке
                th.trigger("reset");
            }, 3000);
        });
        return false;
    ...
    $('.partners').owlCarousel({
        ...
        responsive: {
            ...
            1200: { //размер экрана больше 1200
                items: 4 //показать 4 слайда
            }
        }
    });
    
    //кнопка вверх
    $(window).scroll(function() { //появление и ищезание кнопки вверх
    ...
    $('.top').click(function() { //сам скролл к началу
        $('html, body').stop().animate({scrollTop: 0}, 'slow');
    });
    
    //прелоадер убрать после загрузки страницы
